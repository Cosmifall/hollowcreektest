...
local OUTSIDE_BUILDING = 'outside'

local function normalizeBuilding(building)
    return building or OUTSIDE_BUILDING
end

QBCore.Functions.CreateCallback('qb-weed:server:getBuildingPlants', function(_, cb, building)
    local buildingPlants = {}
    local zone = normalizeBuilding(building)

    MySQL.query('SELECT * FROM house_plants WHERE building = ?', { zone }, function(plants)
        for i = 1, #plants, 1 do
...
RegisterNetEvent('qb-weed:server:placePlant', function(coords, sort, currentHouse)
    ...
    local zone = normalizeBuilding(currentHouse)

    MySQL.insert('INSERT INTO house_plants (building, coords, gender, sort, plantid) VALUES (?, ?, ?, ?, ?)',
        { zone, coords, gender, sort, math.random(111111, 999999) })
    TriggerClientEvent('qb-weed:client:refreshHousePlants', -1, zone)
end)

RegisterNetEvent('qb-weed:server:removeDeathPlant', function(building, plantId)
    local zone = normalizeBuilding(building)
    MySQL.query('DELETE FROM house_plants WHERE plantid = ? AND building = ?', { plantId, zone })
    TriggerClientEvent('qb-weed:client:refreshHousePlants', -1, zone)
end)
...
RegisterNetEvent('qb-weed:server:harvestPlant', function(house, amount, plantName, plantId)
    ...
    if weedBag ~= nil and weedBag.amount >= sndAmount then
        local zone = normalizeBuilding(house)
        local result = MySQL.query.await('SELECT * FROM house_plants WHERE plantid = ? AND building = ?', { plantId, zone })
        if result[1] ~= nil then
            local seedItem = 'weed_' .. plantName .. '_seed'
            local weedItem = 'weed_' .. plantName

            exports['qb-inventory']:AddItem(src, seedItem, amount, false, false, 'qb-weed:server:harvestPlant')
            exports['qb-inventory']:AddItem(src, weedItem, sndAmount, false, false, 'qb-weed:server:harvestPlant')
            exports['qb-inventory']:RemoveItem(src, 'empty_weed_bag', sndAmount, false, 'qb-weed:server:harvestPlant')
            MySQL.query('DELETE FROM house_plants WHERE plantid = ? AND building = ?', { plantId, zone })

            local harvestMessage = ('Harvest successful. Added %sx %s and %sx %s.'):format(amount, seedItem, sndAmount, weedItem)
            TriggerClientEvent('QBCore:Notify', src, harvestMessage, 'success', 5000)
            TriggerClientEvent('qb-weed:client:refreshHousePlants', -1, zone)
        else
...
RegisterNetEvent('qb-weed:server:foodPlant', function(house, amount, plantName, plantId)
    ...
    if not Player then return end
    local zone = normalizeBuilding(house)
    local plantStats = MySQL.query.await('SELECT * FROM house_plants WHERE building = ? AND sort = ? AND plantid = ?', { zone, plantName, tostring(plantId) })
    ...
    MySQL.update('UPDATE house_plants SET food = ? WHERE building = ? AND plantid = ?', { updatedFood, zone, plantId })
