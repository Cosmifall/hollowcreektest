...
local function loadPlantsForBuilding(building)
    QBCore.Functions.TriggerCallback('qb-weed:server:getBuildingPlants', function(plants)
        housePlants[building] = plants
        if currentHouse == building then
            spawnHousePlants()
        end
    end, building)
end
...
local function setBuilding(building)
    if currentHouse == building then return end

    if plantSpawned and currentHouse and housePlants[currentHouse] then
        for k in pairs(housePlants[currentHouse]) do
            local plantData = {
                ['plantCoords'] = json.decode(housePlants[currentHouse][k].coords),
            }
    ...
    if not housePlants[currentHouse] then
        loadPlantsForBuilding(currentHouse)
    else
        spawnHousePlants()
    end
end
...
local function updateHousePlants(leftHouse)
    ...
    CreateThread(function()
        local currentPlants = housePlants[currentHouse]
        if not currentPlants then
            plantSpawned = false
            return
        end

        for k in pairs(currentPlants) do
            local plantData = {
                ['plantCoords'] = json.decode(currentPlants[k].coords),
            }

            for _, stage in pairs(QBWeed.Plants[currentPlants[k].sort]['stages']) do
                local oldPlant = GetClosestObjectOfType(plantData['plantCoords'].x, plantData['plantCoords'].y, plantData['plantCoords'].z, 3.5, GetHashKey(stage), false, false, false)
                if oldPlant ~= 0 then
                    DeleteObject(oldPlant)
                end
...
local function getPlantData()
    if not currentHouse or not housePlants[currentHouse] or not housePlants[currentHouse][closestPlant] then return nil end
    ...
    CreateThread(function()
        while currentHouse do
            ...
            if currentPlants then
                for k in pairs(currentPlants) do
                    local plantCoords = json.decode(currentPlants[k].coords)
...
RegisterNetEvent('qb-weed:client:placePlant', function(type, item)
    if not currentHouse then setBuilding(OUTSIDE_BUILDING) end
    ...
    QBCore.Functions.Progressbar('plant_weed_plant', Lang:t('text.planting'), 8000, false, true, {
        ...
        ClearPedTasks(ped)
        TriggerServerEvent('qb-weed:server:placePlant', json.encode(plantData['plantCoords']), type, currentHouse or OUTSIDE_BUILDING)
