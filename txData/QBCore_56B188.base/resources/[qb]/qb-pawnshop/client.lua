...
local function getPawnShopData(shopId)
    if shopId and Config.PawnItems[shopId] then
        return Config.PawnItems[shopId], shopId
    end

    local playerCoords = GetEntityCoords(PlayerPedId())
    local closestShopId = 1
    local closestDist

    for id, value in pairs(Config.PawnLocation) do
    ...
    return Config.PawnItems[closestShopId] or Config.PawnItems[1], closestShopId
end
...
RegisterNetEvent('qb-pawnshop:client:openMenu', function(data)
    local shopItems, shopId = getPawnShopData(data and data.shopId)
    ...
    if sellingItem then
        ...
        if tonumber(sellingItem.amount) > 0 then
            if tonumber(sellingItem.amount) <= item.amount then
                TriggerServerEvent('qb-pawnshop:server:sellPawnItems', item.name, sellingItem.amount, item.price, item.shopId)
            else
