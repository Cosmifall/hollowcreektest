...
RegisterNetEvent('qb-pawnshop:server:sellPawnItems', function(itemName, itemAmount, itemPrice, shopId)
    local src = source
    local Player = exports['qb-core']:GetPlayer(src)
    local shopItems = Config.PawnItems[tonumber(shopId)] or Config.PawnItems[1]
    ...
    if not allowedItem then
        exploitBan(src, 'sellPawnItems Invalid Item')
        return
    end

    local totalPrice = (tonumber(itemAmount) * price)
    ...
    TriggerClientEvent('qb-pawnshop:client:openMenu', src, { shopId = shopId })
end)
