...
-- Save armor on player unload so it persists through restarts
RegisterNetEvent('QBCore:Client:OnPlayerUnload', function()
    local armor = GetPedArmour(PlayerPedId())
    TriggerServerEvent('hud:server:SaveArmor', armor)
end)
...
-- Periodically save armor to metadata so it persists through disconnects and restarts
CreateThread(function()
    while true do
        Wait(30000)
        if LocalPlayer.state.isLoggedIn then
            TriggerServerEvent('hud:server:SaveArmor', GetPedArmour(PlayerPedId()))
        end
    end
end)
