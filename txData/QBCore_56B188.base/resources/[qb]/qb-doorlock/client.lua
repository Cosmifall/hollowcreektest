...
local function GetFreeAimedDoorEntity()
	local entity = GetEntityPlayerIsFreeAimingAt(PlayerId())
	if not entity or entity == 0 then return false, 0 end
	local entityType = GetEntityType(entity)
	if entityType ~= 3 then return false, 0 end
	return true, entity
end

local function FindNearbyDoorEntity()
	local playerCoords = GetEntityCoords(PlayerPedId())
	local models = { `v_ilev_gb_vaubar`, `hei_prop_heist_sec_door`, `v_ilev_gb_vauldr` }
	for _, model in ipairs(models) do
		local entity = GetClosestObjectOfType(playerCoords.x, playerCoords.y, playerCoords.z, 5.0, model, false, false, false)
		if entity and entity ~= 0 then
			return entity
		end
	end
	local _, entity = GetFreeAimedDoorEntity()
	return entity
end

local function ForceUnlockDoorEntity(entity)
	if not entity or entity == 0 then return false end
	if GetEntityType(entity) ~= 3 then return false end

	local entityCoords = GetEntityCoords(entity)
	local doorHash = ('door_lockpick_%s_%s_%s_%s'):format(GetEntityModel(entity), math.floor(entityCoords.x * 100), math.floor(entityCoords.y * 100), math.floor(entityCoords.z * 100))
	if not IsDoorRegisteredWithSystem(doorHash) then
		AddDoorToSystem(doorHash, GetEntityModel(entity), entityCoords, false, false, false)
	end
	DoorSystemSetDoorState(doorHash, 0, false, false)
	SetEntityHeading(entity, (GetEntityHeading(entity) + 90.0) % 360.0)
	return true
end
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
	...
	if success then
		...
		local entityHit = FindNearbyDoorEntity()
		if entityHit and entityHit ~= 0 then
			doorOpened = ForceUnlockDoorEntity(entityHit)
		end
		...
		-- Unlock the door in the shared door system as well
